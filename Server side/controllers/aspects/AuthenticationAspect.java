...
public class AuthenticationAspect {
    ...
    /* *****************************************************************************************************************
    @Deprecated
    @Around("execution(* com.miko.couponsystemfullstack.controllers.clients.AdminController.*(..))")
        public ResponseEntity<?> authenticateAdmin(ProceedingJoinPoint joinPoint) throws Throwable {
            String token = (String) joinPoint.getArgs()[0];
            if (sessionsMap.containsKey(token)) {
            Session current = sessionsMap.get(token);
            ClientService service = current.getClientService();
            if (service instanceof AdminService && (System.currentTimeMillis() - current.getLastSeen()) < TIMEOUT) {
                current.setLastSeen(System.currentTimeMillis());
                sessionsMap.put(token, current);
    ...
    @Deprecated
    @Around("execution(* com.miko.couponsystemfullstack.controllers.clients.CompanyController.*(..))")
        public ResponseEntity<?> authenticateCompany(ProceedingJoinPoint joinPoint) throws Throwable {
            String token = (String) joinPoint.getArgs()[0];
         if (sessionsMap.containsKey(token)) {
            Session current = sessionsMap.get(token);
            ClientService service = current.getClientService();
            if (service instanceof CompanyService && (System.currentTimeMillis() - current.getLastSeen()) < TIMEOUT) {
                current.setLastSeen(System.currentTimeMillis());
                sessionsMap.put(token, current);
                return (ResponseEntity<?>) joinPoint.proceed();
            } else {
    ...
    @Deprecated
    @Around("execution(* com.miko.couponsystemfullstack.controllers.clients.CustomerController.*(..))")
        public ResponseEntity<?> authenticateCustomer(ProceedingJoinPoint joinPoint) throws Throwable {
        String token = (String) joinPoint.getArgs()[0];
        if (sessionsMap.containsKey(token)) {
            Session current = sessionsMap.get(token);
            ClientService service = current.getClientService();
            if (service instanceof CustomerService && (System.currentTimeMillis() - current.getLastSeen()) < TIMEOUT) {
                current.setLastSeen(System.currentTimeMillis());
                sessionsMap.put(token, current);
                return (ResponseEntity<?>) joinPoint.proceed();
            } else {
